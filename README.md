...
| llama-cpp-laurent-qwen4exp-rocmfpx | llama.cpp with adaptive speculative decoding (--spec-draft-adaptive) and a Vulkan backend tuned for AMD Strix Halo. 4.7x on structured output, 1.9x mainline prefill on MoE. |
| llama-cpp-apepojken                | llama.cpp fork with Qwen4Exp spec-decoding, MTP, ROCm + Vulkan                                                                                                               |
...
| enableHiDPIHack | bool                    | `false`       | Scale Ghidra GUI automatically                                                                                       |
| enableRPC       | bool                    | `false`       | Enable ghidra-rpc CLI                                                                                                |
| extensions      | functionTo (listOf pkg) | `_: []`       | Function passed to the selected package's `withExtensions` that returns a list of Ghidra extensions                  |
