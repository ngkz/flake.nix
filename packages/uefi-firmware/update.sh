#!/usr/bin/env bash
# Update uefi-firmware from the upstream GitHub repo (theopolis/uefi-firmware-
# parser). Version source is PyPI (the repo tags match PyPI versions); the
# source hash comes from nix-prefetch-github at the v<version> tag.
set -euo pipefail

cd "$(dirname "${BASH_SOURCE[0]}")"

pname=uefi-firmware
pypiname=uefi-firmware
owner=theopolis
repo=uefi-firmware-parser

current=$(sed -n 's/.*version = "\(.*\)";.*/\1/p' default.nix)
latest=$(curl -sfL "https://pypi.org/pypi/$pypiname/json" | jq -r '.info.version')

if [[ $current == "$latest" ]]; then
    echo "$pname is up-to-date: $latest"
    exit 0
fi

sed -i "s/version = \"$current\"/version = \"$latest\"/" default.nix
newhash=$(nix-prefetch-github --json "$owner" "$repo" --rev "v$latest" | jq -r .hash)
sed -i "s@\(sha256\|hash\) = \".*\"@hash = \"$newhash\"@" default.nix

echo "$pname updated: $current -> $latest"
